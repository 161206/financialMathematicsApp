      ...
      <v-card class=" primary--text">
        <v-layout row wrap justify-center class="pa-1">
          <v-flex xs12 sm8 md6 lg6 xl6>
            <div><h2>Annuity Payment Factor Calculator (PV)</h2></div>
          </v-flex>
        </v-layout>
        <br/>
        <v-layout row wrap justify-center class="pa-1">
...
export default {
  ...
  computed:{
        paymentfactor: function () {
          const paymentfactor  =this.sharedState.rate/(1-Math.pow(1+this.sharedState.rate/100,-this.sharedState.time));
          return Math.round(paymentfactor * 100) / 100;
        }
      }
